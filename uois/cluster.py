...
def gaussian_kernel(x, y, sigma):
    ...
    """
    return torch.exp(-0.5 / (sigma**2) * euclidean_distances(x, y) ** 2)
...
class MeanShift(ABC):
    ...
    def __init__(
        self, num_seeds=100, max_iters=10, epsilon=1e-2, h=1.0, batch_size=None
    ):
    ...
    def seed_hill_climbing(self, X, Z):
        ...
        for _iter in range(self.max_iters):
            ...
            for i in range(0, m, self.batch_size):
                W = self.kernel(
                    Z[i : i + self.batch_size], X, self.h
                )  # Shape: [batch_size x n]
...
class GaussianMeanShift(MeanShift):
    ...
    def mean_shift_smart_init(self, X, sigmas=None):
        ...
        ]  # Shape: [n//subsample_factor x d]
        if sigmas is not None:
            subsampled_sigmas = sigmas[
                :: self.subsample_factor
            ]  # Shape: [n//subsample_factor]
        ...
        # Run mean shift
        seed_cluster_labels, updated_seeds = self.mean_shift_with_seeds(
            subsampled_X, seeds
        )
        ...
        uniq_cluster_centers = torch.zeros(
            (uniq_labels.shape[0], updated_seeds.shape[1]),
            dtype=torch.float,
            device=updated_seeds.device,
        )
        for i, label in enumerate(uniq_labels):
            uniq_cluster_centers[i, :] = updated_seeds[
                seed_cluster_labels == i, :
            ].mean(dim=0)
