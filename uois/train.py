...
class Trainer(ABC):
    ...
    def save(self, name=None, save_dir=None):
        ...
        filename = (
            ...
            + str(self.iter_num)
            + "_"
            + str(self.model_wrapper.config["feature_dim"])
            + "c"
            + "_checkpoint.pth"
        )
...
class DSNTrainer(Trainer):
    def setup(self):
        ...
        self.tb_writer = SummaryWriter(
            self.config["tb_directory"], flush_secs=self.config["flush_secs"]
        )
    ...
    def train(self, num_epochs, data_loader):
        ...
        for epoch_iter in range(num_epochs):
            for i, batch in enumerate(data_loader):
                ...
                center_offset_loss = self.losses["co_loss"](
                    center_offsets, center_offset_labels, foreground_labels
                )

                separation_loss = torch.tensor(0.0).to(self.device)
                ...
                for j in range(N):
                    ...
                    M_logits = self.model_wrapper.construct_M_logits(
                        predicted_centers, gt_centers
                    )
                    M_gt = ls.create_M_GT(foreground_labels[j])
                    separation_loss = separation_loss + self.losses["sep_loss"](
                        M_logits.unsqueeze(0),
                        M_gt.unsqueeze(0),
                    ...
                    X_fg = X[fg_mask_j][
                        :: self.model_wrapper.config["subsample_factor"], ...
                    ]  # Shape: [num_fg_pixels//subsample_factor x 3]
                    Y_fg = foreground_labels[j, fg_mask_j][
                        :: self.model_wrapper.config["subsample_factor"]
                    ]  # Shape: [num_fg_pixels//subsample_factor]
                    X_fg = X_fg.to(f"cuda:{j % NUM_GPUS}")
                    Y_fg = Y_fg.to(f"cuda:{j % NUM_GPUS}")
                    X_I, Y_I = smart_random_sample_indices(
                        X_fg, Y_fg, self.config["num_seeds_training"]
                    )
                    for l in range(L):
                        X_I = hill_climb_one_iter(
                            X_I, X_fg, self.model_wrapper.config["sigma"]
                        )
                        cluster_loss = cluster_loss + self.losses["cl_loss"](
                            X_I, Y_I, X_fg, Y_fg
                        ).to(self.device)
                ...
                if self.iter_num % self.config["iter_collect"] == 0:
                    ...
                    self.tb_writer.add_scalar(
                        "Loss/Foreground", info["FG loss"], self.iter_num
                    )
                    self.tb_writer.add_scalar(
                        "Loss/Center Offset", info["Center Offset loss"], self.iter_num
                    )
                    self.tb_writer.add_scalar(
                        "Loss/Cluster", info["Cluster loss"], self.iter_num
                    )
                    self.tb_writer.add_scalar(
                        "Loss/Separation", info["Separation loss"], self.iter_num
                    )
                    self.tb_writer.add_scalar(
                        "Time/per iter", info["Batch Time"], self.iter_num
                    )
                    self.tb_writer.add_scalar(
                        "Time/data fetch", info["Data Time"], self.iter_num
                    )
