        ...
        with torch.no_grad():
            ...
            for i in range(N):
                clustered_img, cluster_centers = self.cluster(
                    batch["xyz"][i], center_offsets[i], fg_mask[i] == OBJECTS_LABEL
                )
...
class UOISNet3D(object):
    ...
    def rrn_preprocess(self, rgb_img, initial_masks):
        ...
        for index, mask_id in enumerate(mask_ids):
            ...
            # Resize
            new_size = (224, 224)
            rgb_crop = F.interpolate(rgb_crop.unsqueeze(0), new_size, mode="bilinear")[
                0
            ]  # Shape: [3 x new_H x new_W]
            rgb_crops[index] = rgb_crop
            mask_crop = F.interpolate(
                mask_crop.unsqueeze(0).unsqueeze(0), new_size, mode="nearest"
            )[
                0, 0
            ]  # Shape: [new_H, new_W]
